...
const drawer = props => {
    ...
    return (
        <div className={drawerStyle.join(' ')}>
            <ul>
                <h1>PetInn</h1>
                <div className={styles.ItemContainer} onClick={props.closeDrawer}><NavigationItem to='#' style={{marginTop: '15px'}}>Quero ser anfitrião</NavigationItem></div>
                <div className={styles.ItemContainer} onClick={props.closeDrawer}><NavigationItem to='#'>Opção 2</NavigationItem></div>
                {signInItems}
            </ul>
        </div>
    )
